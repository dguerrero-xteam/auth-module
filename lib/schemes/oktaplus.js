...
export default class OktaPlusScheme {
  ...
  async fetchUser () {
    ...
    console.log('endpoint', this.options.userinfo_endpoint)
  ...
  async _handleCallback (uri) {
    ...
    if (this.options.response_type.includes('code') && parsedQuery.code && this.options.token_endpoint) {
      const opts = {
      ...
      const url = this.options.token_endpoint + '?' + encodeQuery(opts)
