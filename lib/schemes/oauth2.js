...
export default class Oauth2Scheme {
  ...
  get _responseType () {
    return Array.isArray(this.options.response_type)
      ? this.options.response_type.join(' ')
      : this.options.response_type
  }
  ...
  _handleLogoutRedirect () {
    const oktaLogoutRequestParams = {
      id_token_hint: this.$auth.$storage.getUniversal('id_token'),
      post_logout_redirect_uri: this.options.redirect_uri_after_logout
    }

    const queryParamsString =
      Object.entries(oktaLogoutRequestParams)
        .map(kv => (kv.join('=')))
        .join('&')

    window.location =
      this.options.logout_endpoint + '?' + queryParamsString
  }

  async logout () {
    this._clearToken()

    if (this.options.redirect_uri_after_logout) {
  ...
  async fetchUser () {
    console.info('fetchUser running')
